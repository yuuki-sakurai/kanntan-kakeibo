...
test('login bootstraps CSRF, sends cookies and uses the rotated token for mutations', async (t) => {
  const calls = []
  t.mock.method(globalThis, 'fetch', async (url, init) => {
    calls.push({ url, init })
    if (url.endsWith('/auth/session')) return Response.json({ user: null, csrfToken: 'before-login' })
    if (url.endsWith('/auth/login')) return Response.json({ user: { id: 1, name: '利用者', email: 'user@example.com' }, csrfToken: 'after-login' })
    return Response.json({ id: 'new-category', name: 'お酒' })
  })
  const api = new HttpExpenseApi('/api/v1')
  await api.login('user@example.com', 'password-12345')
  await api.createCategory('お酒')
  assert.equal(calls[1].init.headers['X-CSRF-TOKEN'], 'before-login')
  assert.equal(calls[2].init.headers['X-CSRF-TOKEN'], 'after-login')
  for (const call of calls) { assert.equal(call.init.credentials, 'include'); assert.equal(call.init.cache, 'no-store') }
})

test('expired sessions and throttling return actionable errors without retrying writes', async (t) => {
  const api = new HttpExpenseApi('/api/v1')
  const mock = t.mock.method(globalThis, 'fetch')
  for (const status of [401, 419, 429]) {
    api.csrfToken = 'token'
    mock.mock.mockImplementation(async () => Response.json({}, { status }))
    await assert.rejects(api.createCategory('お酒'), error => error.status === status)
  }
